...
case class Normal[U[_], V, S](mu: Node[U, V, S], sigma: Node[U, V, S])(implicit vt: ValueOps[U, V], ops: ContainerOps.Aux[U, S], so: ScalarOps[V, Double]) extends Distribution[U, V, S] {
...
case class NormalSample[U[_], V, S](mu: Node[U, V, S], sigma: Node[U, V, S])(implicit val vt: ValueOps[U, V], val ops: ContainerOps.Aux[U, S], val so: ScalarOps[V, Double], model: Model)
...
case class NormalObservation[U[_], V, S](mu: Node[U, V, S], sigma: Node[U, V, S], value: U[V])(implicit val vt: ValueOps[U, V], val ops: ContainerOps.Aux[U, S], val so: ScalarOps[V, Double])
