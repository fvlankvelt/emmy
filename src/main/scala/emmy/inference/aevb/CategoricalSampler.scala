...
//    val cVal:     Evaluable[Int],
//    val logP:     Evaluable[Double],
...
//   */
//  //@formatter:on
//  def update(ctx: SampleContext, rho: Double): (CategoricalSampler, Double) = {
//    val index = cVal(ctx)
//    val gradLogQ = gradLogTheta(index)
//    val lp = logP(ctx)
//    val newOffset = (1.0 - rho) * offset + rho * lp
//
//    val logQ = math.log(thetas(index)) - math.log(thetas.sum)
//    val deltaLog = logQ - (lp - newOffset)
...
//
//  // draw from P?
//  def sample(ec: SampleContext): Int = {
