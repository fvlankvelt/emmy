...
class CategoricalSamplerSpec extends FlatSpec {
  ...
  "The AEVB sampler" should "determine \\mu close to the exact solution" in {
    ...
    val logp = new Expression[Id, Double, Any] {
      ...
      val logs = Seq(math.log(0.87), math.log(0.13))
    ...
    val newSampler = (0 until 2000).foldLeft(sampler) {
      case (s, idx) ⇒
...
//        println(s"delta: ${update._2}")
    ...
    // (3/2) / (2/3) = 9/4 ~ 2.25
    println()
    println(newSampler.offset)
    println(newSampler.thetas)
    println(newSampler.thetas(0) / newSampler.thetas(1))
